...
public class MathUtil {
    ...
    public static double dLdW_NORM = 10.0;
    /**
    ...
    public static void updateWeightMatrix(Matrix dLdW, double eta, Matrix w, Matrix v,
        ...
        try {
            ...
            // update weight matrix
            // W(n, m) = W(n, m) + dw(m, n) without momentum
            // W(n, m) = W(n, m) + v  with momentum
            MTX.addInplace(w, v);
    ...
    public static double decayStepFn(double minY, double maxY, double decay, int stepSize, int count) {
        ...
        // y : minY to maxY
        double y = Math.max(minY, maxY - decay * nSteps);
    ...
    /**
     Rescale x to min y to max y

     @param x    value within known range minX to maxX
     @param minX min x range
     @param maxX max x range
     @param minY min x range
     @param maxY max x range
     @return y rescaled x in range minY to maxY
     */
    public static double rescale(double x, double minX, double maxX, double minY, double maxY) {
        double y = minY + (x - minX) * (maxY - minY) / (maxX - minX);
        return y;
    }
